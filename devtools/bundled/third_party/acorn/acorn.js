...
function c(t, i) {
    return t < 65 ? 36 === t : t < 91 || (t < 97 ? 95 === t : t < 123 || (t <= 65535 ? t >= 170 && o.test(String.fromCharCode(t)) : !1 !== i && p(t, e)))
}
function l(i, s) {
    return i < 48 ? 36 === i : i < 58 || !(i < 65) && (i < 91 || (i < 97 ? 95 === i : i < 123 || (i <= 65535 ? i >= 170 && h.test(String.fromCharCode(i)) : !1 !== s && (p(i, e) || p(i, t)))))
}
var u = function(t, e) {
    void 0 === e && (e = {}),
    this.label = t,
...
function k(t, e, i) {
    ...
    for (var s = e; s < i; s++) {
        ...
        if (_(r))
            return s < i - 1 && 13 === r && 10 === t.charCodeAt(s + 1) ? s + 2 : s + 1
    }
    return -1
}
var S = /[\u1680\u2000-\u200a\u202f\u205f\u3000\ufeff]/
...
function M(t) {
    ...
    for (var i in O)
        e[i] = t && A(t, i) ? t[i] : O[i];
    if ("latest" === e.ecmaVersion ? e.ecmaVersion = 1e8 : null == e.ecmaVersion ? (!B && "object" == typeof console && console.warn && (B = !0,
...
var q = function(t, e, i) {
    ...
    this.keywords = N(a[t.ecmaVersion >= 6 ? 6 : "module" === t.sourceType ? "5module" : 5]);
    var r = "";
    !0 !== t.allowReserved && (r = s[t.ecmaVersion >= 6 ? 6 : 5 === t.ecmaVersion ? 5 : 3],
    ...
    this.curLine = this.input.slice(0, this.lineStart).split(y).length) : (this.pos = this.lineStart = 0,
    this.curLine = 1),
    this.type = v.eof,
    this.value = null,
...
j.canAwait.get = function() {
    for (var t = this.scopeStack.length - 1; t >= 0; t--) {
        ...
        if (e.inClassFieldInit || e.flags & F)
            return !1;
        if (2 & e.flags)
            return (4 & e.flags) > 0
    }
...
G.strictDirective = function(t) {
    ...
    for (; ; ) {
        ...
        if ("use strict" === (e[1] || e[2])) {
            ...
            return ";" === r || "}" === r || y.test(i[0]) && !(/[(`.[+\-/*%<>=,?^&]/.test(r) || "!" === r && "=" === this.input.charAt(s + 1))
        }
        t += e[0].length,
        C.lastIndex = t,
        t += C.exec(this.input)[0].length,
...
G.canInsertSemicolon = function() {
    return this.type === v.eof || this.type === v.braceR || y.test(this.input.slice(this.lastTokEnd, this.start))
}
,
...
z.isLet = function(t) {
    ...
    if (t)
        return !1;
    if (123 === s || s > 55295 && s < 56320)
        return !0;
    if (c(s, !0)) {
        for (var r = i + 1; l(s = this.input.charCodeAt(r), !0); )
            ++r;
...
z.isAsyncFunction = function() {
    if (this.options.ecmaVersion < 8 || !this.isContextual("async"))
        return !1;
    C.lastIndex = this.pos;
    var t, e = C.exec(this.input), i = this.pos + e[0].length;
    return !(y.test(this.input.slice(this.pos, i)) || "function" !== this.input.slice(i, i + 8) || i + 8 !== this.input.length && (l(t = this.input.charCodeAt(i + 8)) || t > 55295 && t < 56320))
}
,
z.parseStatement = function(t, e, i) {
    var s, r = this.type, a = this.startNode();
    switch (this.isLet(t) && (r = v._var,
    s = "let"),
    r) {
    case v._break:
    case v._continue:
...
z.parseBreakContinueStatement = function(t, e) {
    var i = "break" === e;
    this.next(),
    this.eat(v.semi) || this.insertSemicolon() ? t.label = null : this.type !== v.name ? this.unexpected() : (t.label = this.parseIdent(),
    this.semicolon());
...
z.parseForStatement = function(t) {
    ...
    if (this.type === v._var || this.type === v._const || i) {
        ...
        (this.type === v._in || this.options.ecmaVersion >= 6 && this.isContextual("of")) && 1 === s.declarations.length ? (this.options.ecmaVersion >= 9 && (this.type === v._in ? e > -1 && this.unexpected(e) : t.await = e > -1),
        this.parseForIn(t, s)) : (e > -1 && this.unexpected(e),
        this.parseFor(t, s))
    }
    ...
    return this.type === v._in || (n = this.options.ecmaVersion >= 6 && this.isContextual("of")) ? (this.options.ecmaVersion >= 9 && (this.type === v._in ? e > -1 && this.unexpected(e) : t.await = e > -1),
...
z.parseFunctionStatement = function(t, e, i) {
    return this.next(),
    this.parseFunction(t, X | (i ? 0 : Z), !1, e)
}
,
...
z.parseReturnStatement = function(t) {
    ...
    this.next(),
    this.eat(v.semi) || this.insertSemicolon() ? t.argument = null : (t.argument = this.parseExpression(),
    this.semicolon()),
...
z.parseThrowStatement = function(t) {
    return this.next(),
    y.test(this.input.slice(this.lastTokEnd, this.start)) && this.raise(this.lastTokEnd, "Illegal newline after throw"),
...
z.parseTryStatement = function(t) {
    ...
    this.type === v._catch) {
        ...
        this.next(),
        this.eat(v.parenL) ? e.param = this.parseCatchClauseParam() : (this.options.ecmaVersion < 10 && this.unexpected(),
        e.param = null,
        this.enterScope(0)),
    ...
    return t.finalizer = this.eat(v._finally) ? this.parseBlock() : null,
...
z.parseBlock = function(t, e, i) {
    ...
    e.body = [],
    this.expect(v.braceL),
    t && this.enterScope(0); this.type !== v.braceR; ) {
...
z.parseVar = function(t, e, i, s) {
    ...
    t.kind = i; ; ) {
        ...
        if (this.parseVarId(r, i),
        this.eat(v.eq) ? r.init = this.parseMaybeAssign(e) : s || "const" !== i || this.type === v._in || this.options.ecmaVersion >= 6 && this.isContextual("of") ? s || "Identifier" === r.id.type || e && (this.type === v._in || this.isContextual("of")) ? r.init = null : this.raise(this.lastTokEnd, "Complex binding patterns require an initialization value") : this.unexpected(),
...
function $(t, e) {
    ...
    "iget" === s && "iset" === r || "iset" === s && "iget" === r || "sget" === s && "sset" === r || "sset" === s && "sget" === r ? (t[i] = "true",
    !1) : !!s || (t[i] = r,
    !1)
}
function J(t, e) {
...
z.parseFunction = function(t, e, i, s, r) {
    this.initFunction(t),
    (this.options.ecmaVersion >= 9 || this.options.ecmaVersion >= 6 && !s) && (this.type === v.star && e & Z && this.unexpected(),
    t.generator = this.eat(v.star)),
    this.options.ecmaVersion >= 8 && (t.async = !!s),
    e & X && (t.id = 4 & e && this.type !== v.name ? null : this.parseIdent(),
    !t.id || e & Z || this.checkLValSimple(t.id, this.strict || t.generator || t.async ? this.treatFunctionsAsVar ? 1 : 2 : 3));
    var a = this.yieldPos
      , n = this.awaitPos
...
z.parseFunctionParams = function(t) {
    this.expect(v.parenL),
    t.params = this.parseBindingList(v.parenR, !1, this.options.ecmaVersion >= 8),
...
z.parseClass = function(t, e) {
    ...
    var s = this.enterClassBody()
      ...
      , a = !1;
    for (r.body = [],
    this.expect(v.braceL); this.type !== v.braceR; ) {
        var n = this.parseClassElement(null !== t.superClass);
        n && (r.body.push(n),
        "MethodDefinition" === n.type && "constructor" === n.kind ? (a && this.raiseRecoverable(n.start, "Duplicate constructor in the same class"),
        a = !0) : n.key && "PrivateIdentifier" === n.key.type && $(s, n) && this.raiseRecoverable(n.key.start, "Identifier '#" + n.key.name + "' has already been declared"))
    }
...
z.parseClassElement = function(t) {
    ...
    if (this.eatContextual("static")) {
        if (e >= 13 && this.eat(v.braceL))
            ...
            i;
        this.isClassElementNameStart() || this.type === v.star ? o = !0 : s = "static"
    }
    if (i.static = o,
    !s && e >= 8 && this.eatContextual("async") && (!this.isClassElementNameStart() && this.type !== v.star || this.canInsertSemicolon() ? s = "async" : a = !0),
    ...
    e < 13 || this.type === v.parenL || "method" !== n || r || a) {
        var p = !i.static && J(i, "constructor")
          , c = p && t;
        p && "method" !== n && this.raise(i.key.start, "Constructor can't have get/set modifier"),
...
z.parseClassElementName = function(t) {
    this.type === v.privateId ? ("constructor" === this.value && this.raise(this.start, "Classes can't have an element named '#constructor'"),
    t.computed = !1,
...
z.parseClassMethod = function(t, e, i, s) {
    ...
    i && this.raise(r.start, "Constructor can't be an async method")) : t.static && J(t, "prototype") && this.raise(r.start, "Classes may not have a static property named prototype");
    ...
    "set" === t.kind && "RestElement" === a.params[0].type && this.raiseRecoverable(a.params[0].start, "Setter cannot use rest params"),
    this.finishNode(t, "MethodDefinition")
}
,
z.parseClassField = function(t) {
    if (J(t, "constructor") ? this.raise(t.key.start, "Classes can't have a field named 'constructor'") : t.static && J(t, "prototype") && this.raise(t.key.start, "Classes can't have a static field named 'prototype'"),
    this.eat(v.eq)) {
...
z.parseClassStaticBlock = function(t) {
    ...
    var e = this.labels;
    for (this.labels = [],
    this.enterScope(320); this.type !== v.braceR; ) {
...
z.parseClassId = function(t, e) {
    this.type === v.name ? (t.id = this.parseIdent(),
...
z.parseExportAllDeclaration = function(t, e) {
    ...
    this.checkExport(e, t.exported, this.lastTokStart)) : t.exported = null),
...
z.parseExport = function(t, e) {
    ...
    if (this.shouldParseExportStatement())
        ...
        "VariableDeclaration" === t.declaration.type ? this.checkVariableExport(e, t.declaration.declarations) : this.checkExport(e, t.declaration.id, t.declaration.id.start),
        t.specifiers = [],
        t.source = null;
    else {
        ...
        this.eatContextual("from"))
            ...
            for (var i = 0, s = t.specifiers; i < s.length; i += 1) {
                ...
                "Literal" === r.local.type && this.raise(r.local.start, "A string literal cannot be used as an exported binding without `from`.")
            }
            t.source = null
        }
        this.semicolon()
...
z.checkExport = function(t, e, i) {
    t && ("string" != typeof e && (e = "Identifier" === e.type ? e.name : e.value),
...
z.parseExportSpecifier = function(t) {
    var e = this.startNode();
    return e.local = this.parseModuleExportName(),
...
z.parseExportSpecifiers = function(t) {
    ...
    for (this.expect(v.braceL); !this.eat(v.braceR); ) {
        if (i)
            i = !1;
        else if (this.expect(v.comma),
        this.afterTrailingComma(v.braceR))
            break;
...
z.parseImport = function(t) {
    return this.next(),
    this.type === v.string ? (t.specifiers = Y,
    t.source = this.parseExprAtom()) : (t.specifiers = this.parseImportSpecifiers(),
    this.expectContextual("from"),
    t.source = this.type === v.string ? this.parseExprAtom() : this.unexpected()),
    this.semicolon(),
...
z.parseImportSpecifier = function() {
    ...
    return t.imported = this.parseModuleExportName(),
    this.eatContextual("as") ? t.local = this.parseIdent() : (this.checkUnreserved(t.imported),
    t.local = t.imported),
...
z.parseImportDefaultSpecifier = function() {
    var t = this.startNode();
    return t.local = this.parseIdent(),
    this.checkLValSimple(t.local, 2),
...
z.parseImportSpecifiers = function() {
    var t = []
      , e = !0;
    if (this.type === v.name && (t.push(this.parseImportDefaultSpecifier()),
    !this.eat(v.comma)))
        return t;
    if (this.type === v.star)
...
z.parseModuleExportName = function() {
    if (this.options.ecmaVersion >= 13 && this.type === v.string) {
        ...
        return T.test(t.value) && this.raise(t.start, "An export name cannot include a lone surrogate."),
        t
    }
    ...
    for (var r = [], a = !0; !this.eat(t); )
        ...
        e && this.type === v.comma)
            ...
            if (this.type === v.ellipsis) {
                ...
                r.push(n),
                this.type === v.comma && this.raiseRecoverable(this.start, "Comma is not permitted after the rest element"),
                this.expect(t);
                break
            }
...
tt.parseAssignableListItem = function(t) {
    ...
    return this.parseBindingListItem(e),
    e
}
,
...
tt.checkLValSimple = function(t, e, i) {
    ...
    switch (t.type) {
    case "Identifier":
        this.strict && this.reservedWordsStrictBind.test(t.name) && this.raiseRecoverable(t.start, (s ? "Binding " : "Assigning to ") + t.name + " in strict mode"),
        s && (2 === e && "let" === t.name && this.raiseRecoverable(t.start, "let is disallowed as a lexically bound name"),
        i && (A(i, t.name) && this.raiseRecoverable(t.start, "Argument name clash"),
        i[t.name] = !0),
    ...
    case "MemberExpression":
        ...
        break;
    case "ParenthesizedExpression":
        return s && this.raiseRecoverable(t.start, "Binding parenthesized expression"),
...
st.braceIsBlock = function(t) {
    ...
    return e === it.f_expr || e === it.f_stat || (t !== v.colon || e !== it.b_stat && e !== it.b_expr ? t === v._return || t === v.name && this.exprAllowed ? y.test(this.input.slice(this.lastTokEnd, this.start)) : t === v._else || t === v.semi || t === v.eof || t === v.parenR || t === v.arrow || (t === v.braceL ? e === it.b_stat : t !== v._var && t !== v._const && t !== v.name && !this.exprAllowed) : !e.isExpr)
}
,
...
st.updateContext = function(t) {
    var e, i = this.type;
    i.keyword && t === v.dot ? this.exprAllowed = !1 : (e = i.updateContext) ? e.call(this, t) : this.exprAllowed = i.beforeExpr
}
,
...
,
v._function.updateContext = v._class.updateContext = function(t) {
    !t.beforeExpr || t === v._else || t === v.semi && this.curContext() !== it.p_stat || t === v._return && y.test(this.input.slice(this.lastTokEnd, this.start)) || (t === v.colon || t === v.braceL) && this.curContext() === it.b_stat ? this.context.push(it.f_stat) : this.context.push(it.f_expr),
    this.exprAllowed = !1
}
,
...
v.name.updateContext = function(t) {
    var e = !1;
    this.options.ecmaVersion >= 6 && t !== v.dot && ("of" === this.value && !this.exprAllowed || "yield" === this.value && this.inGeneratorContext()) && (e = !0),
    this.exprAllowed = e
}
;
var rt = q.prototype;
...
rt.checkPropClash = function(t, e, i) {
    if (!(this.options.ecmaVersion >= 9 && "SpreadElement" === t.type || this.options.ecmaVersion >= 6 && (t.computed || t.method || t.shorthand))) {
        ...
        var a = t.kind;
        if (this.options.ecmaVersion >= 6)
            "__proto__" === s && "init" === a && (e.proto && (i ? i.doubleProto < 0 && (i.doubleProto = r.start) : this.raiseRecoverable(r.start, "Redefinition of __proto__ property")),
            e.proto = !0);
        else {
            var n = e[s = "$" + s];
            if (n)
                ("init" === a ? this.strict && n.init || n.get || n.set : n.init || n[a]) && this.raiseRecoverable(r.start, "Redefinition of property");
            else
                n = e[s] = {
                    init: !1,
                    get: !1,
...
rt.parseMaybeAssign = function(t, e, i) {
    ...
    a = e.trailingComma,
    n = e.doubleProto,
    e.parenthesizedAssign = e.trailingComma = -1) : (e = new W,
    s = !0);
    var o = this.start
      , h = this.startLoc;
...
rt.parseExprOp = function(t, e, i, s, r) {
    ...
    if (null != a && (!r || this.type !== v._in) && a > s) {
        ...
        return (n && this.type === v.coalesce || o && (this.type === v.logicalOR || this.type === v.logicalAND)) && this.raiseRecoverable(this.start, "Logical expressions and coalesce expressions cannot be mixed. Wrap either by parentheses"),
...
rt.parseMaybeUnary = function(t, e, i, s) {
    ...
    if (this.isContextual("await") && this.canAwait)
        ...
        if (r = this.parseExprSubscripts(t, s),
        ...
        (s || 0 === this.privateNameStack.length) && this.options.checkPrivateFields && this.unexpected(),
        r = this.parsePrivateIdent(),
        this.type !== v._in && this.unexpected();
    return i || !this.eat(v.starstar) ? r : e ? void this.unexpected(this.lastTokStart) : this.buildBinary(a, n, r, this.parseMaybeUnary(null, !1, !1, s), "**", !1)
}
,
rt.parseExprSubscripts = function(t, e) {
    ...
    return t && "MemberExpression" === a.type && (t.parenthesizedAssign >= a.start && (t.parenthesizedAssign = -1),
...
rt.parseSubscript = function(t, e, i, s, r, a, n) {
    ...
    if (p || h && this.type !== v.parenL && this.type !== v.backQuote || this.eat(v.dot)) {
        ...
        c.object = t,
        p ? (c.property = this.parseExpression(),
        this.expect(v.bracketR)) : this.type === v.privateId && "Super" !== t.type ? c.property = this.parsePrivateIdent() : c.property = this.parseIdent("never" !== this.options.allowReserved),
        c.computed = !!p,
        o && (c.optional = h),
        ...
        if (r && !h && this.shouldParseAsyncArrow())
            return this.checkPatternErrors(l, !1),
            this.checkYieldAwaitInDefaultParams(),
...
rt.parseExprAtom = function(t, e, i) {
    ...
    case v._super:
        ...
        s = this.startNode(),
        this.next(),
        this.type !== v.parenL || this.allowDirectSuper || this.raise(s.start, "super() call outside constructor of a subclass"),
        this.type !== v.dot && this.type !== v.bracketL && this.type !== v.parenL && this.unexpected(),
    ...
    case v.name:
        var a = this.start
          ...
          , o = this.containsEsc
          , h = this.parseIdent(!1);
        if (this.options.ecmaVersion >= 8 && !o && "async" === h.name && !this.canInsertSemicolon() && this.eat(v._function))
            return this.overrideContext(it.f_expr),
            this.parseFunction(this.startNodeAt(a, n), 0, !1, !0, e);
        if (r && !this.canInsertSemicolon()) {
            if (this.eat(v.arrow))
                return this.parseArrowExpression(this.startNodeAt(a, n), [h], !1, e);
            if (this.options.ecmaVersion >= 8 && "async" === h.name && this.type === v.name && !o && (!this.potentialArrowInForAwait || "of" !== this.value || this.containsEsc))
                return h = this.parseIdent(!1),
    ...
    case v._true:
    case v._false:
        return (s = this.startNode()).value = this.type === v._null ? null : this.type === v._true,
    ...
    case v.parenL:
        ...
        return t && (t.parenthesizedAssign < 0 && !this.isSimpleAssignTarget(l) && (t.parenthesizedAssign = c),
    ...
    case v.braceL:
        return this.overrideContext(it.b_expr),
        this.parseObj(!1, t);
    case v._function:
        return s = this.startNode(),
        this.next(),
...
rt.parseExprImport = function(t) {
    ...
    return this.type !== v.parenL || t ? this.type === v.dot ? (e.meta = i,
    this.parseImportMeta(e)) : void this.unexpected() : this.parseDynamicImport(e)
}
,
...
rt.parseParenAndDistinguishExpression = function(t, e) {
    ...
    if (this.options.ecmaVersion >= 6) {
        this.next();
        var n, o = this.start, h = this.startLoc, p = [], c = !0, l = !1, u = new W, d = this.yieldPos, f = this.awaitPos;
        for (this.yieldPos = 0,
        this.awaitPos = 0; this.type !== v.parenR; ) {
            if (c ? c = !1 : this.expect(v.comma),
        ...
        p.length > 1 ? ((i = this.startNodeAt(o, h)).expressions = p,
        this.finishNodeAt(i, "SequenceExpression", m, x)) : i = p[0]
    } else
        i = this.parseParenExpression();
    if (this.options.preserveParens) {
        var g = this.startNodeAt(s, r);
        return g.expression = i,
...
rt.parseNew = function() {
    ...
    var s = this.start
      , r = this.startLoc;
    return t.callee = this.parseSubscripts(this.parseExprAtom(null, !1, !0), s, r, !0, !1),
    this.eat(v.parenL) ? t.arguments = this.parseExprList(v.parenR, this.options.ecmaVersion >= 8, !1) : t.arguments = nt,
...
rt.parseTemplateElement = function(t) {
    var e = t.isTagged
      , i = this.startNode();
    return this.type === v.invalidTemplate ? (e || this.raiseRecoverable(this.start, "Bad escape sequence in untagged template literal"),
    i.value = {
        raw: this.value,
        cooked: null
    }) : i.value = {
...
rt.isAsyncProp = function(t) {
    return !t.computed && "Identifier" === t.key.type && "async" === t.key.name && (this.type === v.name || this.type === v.num || this.type === v.string || this.type === v.bracketL || this.type.keyword || this.options.ecmaVersion >= 9 && this.type === v.star) && !y.test(this.input.slice(this.lastTokEnd, this.start))
}
,
...
rt.parseProperty = function(t, e) {
    ...
    if (this.options.ecmaVersion >= 9 && this.eat(v.ellipsis))
        return t ? (n.argument = this.parseIdent(!1),
        this.type === v.comma && this.raiseRecoverable(this.start, "Comma is not permitted after the rest element"),
        this.finishNode(n, "RestElement")) : (n.argument = this.parseMaybeAssign(!1, e),
        this.type === v.comma && e && e.trailingComma < 0 && (e.trailingComma = this.start),
    ...
    !t && !o && this.options.ecmaVersion >= 8 && !i && this.isAsyncProp(n) ? (s = !0,
    i = this.options.ecmaVersion >= 9 && this.eat(v.star),
    this.parsePropertyName(n)) : s = !1,
...
rt.parsePropertyValue = function(t, e, i, s, r, a, n, o) {
    ...
    this.eat(v.colon) ? (t.value = e ? this.parseMaybeDefault(this.start, this.startLoc) : this.parseMaybeAssign(!1, n),
    t.kind = "init") : this.options.ecmaVersion >= 6 && this.type === v.parenL ? (e && this.unexpected(),
    t.kind = "init",
    t.method = !0,
    t.value = this.parseMethod(i, s)) : e || o || !(this.options.ecmaVersion >= 5) || t.computed || "Identifier" !== t.key.type || "get" !== t.key.name && "set" !== t.key.name || this.type === v.comma || this.type === v.braceR || this.type === v.eq ? this.options.ecmaVersion >= 6 && !t.computed && "Identifier" === t.key.type ? ((i || s) && this.unexpected(),
    this.checkUnreserved(t.key),
    "await" !== t.key.name || this.awaitIdentPos || (this.awaitIdentPos = r),
    t.kind = "init",
    e ? t.value = this.parseMaybeDefault(r, a, this.copyNode(t.key)) : this.type === v.eq && n ? (n.shorthandAssign < 0 && (n.shorthandAssign = this.start),
    t.value = this.parseMaybeDefault(r, a, this.copyNode(t.key))) : t.value = this.copyNode(t.key),
    t.shorthand = !0) : this.unexpected() : ((i || s) && this.unexpected(),
...
rt.parseMethod = function(t, e, i) {
    ...
    this.expect(v.parenL),
    s.params = this.parseBindingList(v.parenR, !1, this.options.ecmaVersion >= 8),
    this.checkYieldAwaitInDefaultParams(),
...
rt.parseExprList = function(t, e, i, s) {
    for (var r = [], a = !0; !this.eat(t); ) {
        ...
        e && this.afterTrailingComma(t))
            break;
        var n = void 0;
        i && this.type === v.comma ? n = null : this.type === v.ellipsis ? (n = this.parseSpread(s),
        s && this.type === v.comma && s.trailingComma < 0 && (s.trailingComma = this.start)) : n = this.parseMaybeAssign(!1, s),
        r.push(n)
    }
    return r
}
...
rt.checkUnreserved = function(t) {
    ...
    this.currentThisScope().inClassFieldInit && "arguments" === s && this.raiseRecoverable(e, "Cannot use 'arguments' in class field initializer"),
    !this.inClassStaticBlock || "arguments" !== s && "await" !== s || this.raise(e, "Cannot use " + s + " in class static initialization block"),
    this.keywords.test(s) && this.raise(e, "Unexpected keyword '" + s + "'"),
    this.options.ecmaVersion < 6 && -1 !== this.input.slice(e, i).indexOf("\\")) || (this.strict ? this.reservedWordsStrict : this.reservedWords).test(s) && (this.inAsync || "await" !== s || this.raiseRecoverable(e, "Cannot use keyword 'await' outside an async function"),
...
rt.parseIdentNode = function() {
    var t = this.startNode();
    return this.type === v.name ? t.name = this.value : this.type.keyword ? (t.name = this.type.keyword,
...
rt.parsePrivateIdent = function() {
    var t = this.startNode();
    return this.type === v.privateId ? t.name = this.value : this.unexpected(),
    this.next(),
    this.finishNode(t, "PrivateIdentifier"),
    this.options.checkPrivateFields && (0 === this.privateNameStack.length ? this.raise(t.start, "Private field '#" + t.name + "' must be declared in an enclosing class") : this.privateNameStack[this.privateNameStack.length - 1].used.push(t)),
    t
}
,
rt.parseYield = function(t) {
    ...
    var e = this.startNode();
    return this.next(),
    this.type === v.semi || this.canInsertSemicolon() || this.type !== v.star && !this.type.startsExpr ? (e.delegate = !1,
    e.argument = null) : (e.delegate = this.eat(v.star),
...
var dt = "ASCII ASCII_Hex_Digit AHex Alphabetic Alpha Any Assigned Bidi_Control Bidi_C Bidi_Mirrored Bidi_M Case_Ignorable CI Cased Changes_When_Casefolded CWCF Changes_When_Casemapped CWCM Changes_When_Lowercased CWL Changes_When_NFKC_Casefolded CWKCF Changes_When_Titlecased CWT Changes_When_Uppercased CWU Dash Default_Ignorable_Code_Point DI Deprecated Dep Diacritic Dia Emoji Emoji_Component Emoji_Modifier Emoji_Modifier_Base Emoji_Presentation Extender Ext Grapheme_Base Gr_Base Grapheme_Extend Gr_Ext Hex_Digit Hex IDS_Binary_Operator IDSB IDS_Trinary_Operator IDST ID_Continue IDC ID_Start IDS Ideographic Ideo Join_Control Join_C Logical_Order_Exception LOE Lowercase Lower Math Noncharacter_Code_Point NChar Pattern_Syntax Pat_Syn Pattern_White_Space Pat_WS Quotation_Mark QMark Radical Regional_Indicator RI Sentence_Terminal STerm Soft_Dotted SD Terminal_Punctuation Term Unified_Ideograph UIdeo Uppercase Upper Variation_Selector VS White_Space space XID_Continue XIDC XID_Start XIDS"
  ...
  , bt = yt + " Dogra Dogr Gunjala_Gondi Gong Hanifi_Rohingya Rohg Makasar Maka Medefaidrin Medf Old_Sogdian Sogo Sogdian Sogd"
  , _t = bt + " Elymaic Elym Nandinagari Nand Nyiakeng_Puachue_Hmong Hmnp Wancho Wcho"
  , kt = _t + " Chorasmian Chrs Diak Dives_Akuru Khitan_Small_Script Kits Yezi Yezidi"
...
var Pt = q.prototype
  , Nt = function(t) {
    this.parser = t,
    this.validFlags = "gim" + (t.options.ecmaVersion >= 6 ? "uy" : "") + (t.options.ecmaVersion >= 9 ? "s" : "") + (t.options.ecmaVersion >= 13 ? "d" : "") + (t.options.ecmaVersion >= 15 ? "v" : ""),
    this.unicodeProperties = wt[t.options.ecmaVersion >= 14 ? 14 : t.options.ecmaVersion],
    this.source = "",
    this.flags = "",
    this.start = 0,
    this.switchU = !1,
...
Nt.prototype.reset = function(t, e, i) {
    ...
    this.source = e + "",
    this.flags = i,
    s && this.parser.options.ecmaVersion >= 15 ? (this.switchU = !0,
    this.switchV = !0,
    this.switchN = !0) : (this.switchU = r && this.parser.options.ecmaVersion >= 6,
    this.switchV = !1,
...
Pt.validateRegExpFlags = function(t) {
    for (var e = t.validFlags, i = t.flags, s = !1, r = !1, a = 0; a < i.length; a++) {
        var n = i.charAt(a);
        -1 === e.indexOf(n) && this.raise(t.start, "Invalid regular expression flag"),
        i.indexOf(n, a + 1) > -1 && this.raise(t.start, "Duplicate regular expression flag"),
        "u" === n && (s = !0),
        "v" === n && (r = !0)
    }
...
Pt.regexp_eatBracedQuantifier = function(t, e) {
    ...
    if (t.eat(123)) {
        var s = 0
          , r = -1;
        if (this.regexp_eatDecimalDigits(t) && (s = t.lastIntValue,
        t.eat(44) && this.regexp_eatDecimalDigits(t) && (r = t.lastIntValue),
        t.eat(125)))
            return -1 !== r && r < s && !e && t.raise("numbers out of order in {} quantifier"),
            !0;
...
Pt.regexp_eatRegExpIdentifierName = function(t) {
    ...
    this.regexp_eatRegExpIdentifierStart(t)) {
        for (t.lastStringValue += V(t.lastIntValue); this.regexp_eatRegExpIdentifierPart(t); )
...
Pt.regexp_eatControlLetter = function(t) {
    var e = t.current();
    return !!Tt(e) && (t.lastIntValue = e % 32,
    t.advance(),
    !0)
}
,
Pt.regexp_eatRegExpUnicodeEscapeSequence = function(t, e) {
    ...
    if (t.eat(117)) {
        if (this.regexp_eatFixedHexDigits(t, 4)) {
            ...
            if (r && a >= 55296 && a <= 56319) {
                ...
                if (t.eat(92) && t.eat(117) && this.regexp_eatFixedHexDigits(t, 4)) {
                    ...
                    if (o >= 56320 && o <= 57343)
                        return t.lastIntValue = 1024 * (a - 55296) + (o - 56320) + 65536,
                        !0
                }
                t.pos = n,
                t.lastIntValue = a
        ...
        if (r && t.eat(123) && this.regexp_eatHexDigits(t) && t.eat(125) && ((i = t.lastIntValue) >= 0 && i <= 1114111))
            return !0;
        r && t.raise("Invalid unicode escape"),
        t.pos = s
    }
    return !1
}
...
Pt.regexp_eatCharacterClassEscape = function(t) {
    ...
    if (function(t) {
        ...
        1;
    var i = !1;
    if (t.switchU && this.options.ecmaVersion >= 9 && ((i = 80 === e) || 112 === e)) {
        var s;
        if (t.lastIntValue = -1,
        t.advance(),
        t.eat(123) && (s = this.regexp_eatUnicodePropertyValueExpression(t)) && t.eat(125))
            return i && 2 === s && t.raise("Invalid property name"),
            s;
...
Pt.regexp_eatUnicodePropertyValueExpression = function(t) {
    ...
    if (this.regexp_eatUnicodePropertyName(t) && t.eat(61)) {
        ...
        if (this.regexp_eatUnicodePropertyValue(t)) {
            ...
            return this.regexp_validateUnicodePropertyNameAndValue(t, i, s),
            1
        }
    }
    if (t.pos = e,
...
Pt.regexp_eatCharacterClass = function(t) {
    if (t.eat(91)) {
        ...
        return t.eat(93) || t.raise("Unterminated character class"),
...
Pt.regexp_classContents = function(t) {
    return 93 === t.current() ? 1 : t.switchV ? this.regexp_classSetExpression(t) : (this.regexp_nonEmptyClassRanges(t),
    1)
}
,
...
Pt.regexp_classSetExpression = function(t) {
    ...
    if (this.regexp_eatClassSetRange(t))
        ...
        for (var s = t.pos; t.eatChars([38, 38]); )
            38 !== t.current() && (e = this.regexp_eatClassSetOperand(t)) ? 2 !== e && (i = 1) : t.raise("Invalid character in character class");
        if (s !== t.pos)
            return i;
...
Pt.regexp_eatClassSetRange = function(t) {
    ...
    if (this.regexp_eatClassSetCharacter(t)) {
        ...
        if (t.eat(45) && this.regexp_eatClassSetCharacter(t)) {
            ...
            return -1 !== i && -1 !== s && i > s && t.raise("Range out of order in character class"),
            !0
        }
        t.pos = e
    }
...
Pt.regexp_eatNestedClass = function(t) {
    ...
    if (t.eat(91)) {
        ...
        if (t.eat(93))
            return i && 2 === s && t.raise("Negated character class may contain strings"),
            s;
        t.pos = e
    }
    if (t.eat(92)) {
...
Pt.regexp_eatClassSetCharacter = function(t) {
    var e = t.pos;
    if (t.eat(92))
        return !(!this.regexp_eatCharacterEscape(t) && !this.regexp_eatClassSetReservedPunctuator(t)) || (t.eat(98) ? (t.lastIntValue = 8,
        !0) : (t.pos = e,
        !1));
    var i = t.current();
    return !(i < 0 || i === t.lookahead() && function(t) {
...
Pt.regexp_eatHexDigits = function(t) {
    ...
    for (t.lastIntValue = 0; Ot(i = t.current()); )
        t.lastIntValue = 16 * t.lastIntValue + Bt(i),
        t.advance();
    return t.pos !== e
}
,
...
Pt.regexp_eatOctalDigit = function(t) {
    var e = t.current();
    return Mt(e) ? (t.lastIntValue = e - 48,
    t.advance(),
    !0) : (t.lastIntValue = 0,
    !1)
}
...
Pt.regexp_eatFixedHexDigits = function(t, e) {
    ...
    for (var s = 0; s < e; ++s) {
        ...
        if (!Ot(r))
            return t.pos = i,
            !1;
        t.lastIntValue = 16 * t.lastIntValue + Bt(r),
        t.advance()
    }
    return !0
}
...
,
"undefined" != typeof Symbol && (Ut[Symbol.iterator] = function() {
    var t = this;
    return {
        next: function() {
            var e = t.getToken();
...
Ut.nextToken = function() {
    ...
    this.pos >= this.input.length ? this.finishToken(v.eof) : t.override ? t.override(this) : void this.readToken(this.fullCharCodeAtPos())
}
,
...
Ut.skipLineComment = function(t) {
    for (var e = this.pos, i = this.options.onComment && this.curPosition(), s = this.input.charCodeAt(this.pos += t); this.pos < this.input.length && !_(s); )
...
Ut.skipSpace = function() {
    t: for (; this.pos < this.input.length; ) {
        ...
        case 47:
            ...
            break;
        default:
            if (!(t > 8 && t < 14 || t >= 5760 && S.test(String.fromCharCode(t))))
                break t;
            ++this.pos
        }
    }
...
Ut.readToken_dot = function() {
    ...
    this.finishToken(v.ellipsis)) : (++this.pos,
    this.finishToken(v.dot))
}
,
Ut.readToken_slash = function() {
    var t = this.input.charCodeAt(this.pos + 1);
    return this.exprAllowed ? (++this.pos,
    this.readRegexp()) : 61 === t ? this.finishOp(v.assign, 2) : this.finishOp(v.slash, 1)
}
,
...
Ut.readToken_pipe_amp = function(t) {
    ...
    return 61 === e ? this.finishOp(v.assign, 2) : this.finishOp(124 === t ? v.bitwiseOR : v.bitwiseAND, 1)
}
,
...
Ut.readToken_plus_min = function(t) {
    var e = this.input.charCodeAt(this.pos + 1);
    return e === t ? 45 !== e || this.inModule || 62 !== this.input.charCodeAt(this.pos + 2) || 0 !== this.lastTokEnd && !y.test(this.input.slice(this.lastTokEnd, this.pos)) ? this.finishOp(v.incDec, 2) : (this.skipLineComment(3),
    this.skipSpace(),
    this.nextToken()) : 61 === e ? this.finishOp(v.assign, 2) : this.finishOp(v.plusMin, 1)
}
,
Ut.readToken_lt_gt = function(t) {
    ...
    61 === this.input.charCodeAt(this.pos + i) ? this.finishOp(v.assign, i + 1) : this.finishOp(v.bitShift, i)) : 33 !== e || 60 !== t || this.inModule || 45 !== this.input.charCodeAt(this.pos + 2) || 45 !== this.input.charCodeAt(this.pos + 3) ? (61 === e && (i = 2),
    this.finishOp(v.relational, i)) : (this.skipLineComment(4),
    this.skipSpace(),
    this.nextToken())
}
,
Ut.readToken_eq_excl = function(t) {
    var e = this.input.charCodeAt(this.pos + 1);
    return 61 === e ? this.finishOp(v.equality, 61 === this.input.charCodeAt(this.pos + 2) ? 3 : 2) : 61 === t && 62 === e && this.options.ecmaVersion >= 6 ? (this.pos += 2,
    this.finishToken(v.arrow)) : this.finishOp(61 === t ? v.eq : v.prefix, 1)
}
,
...
Ut.readToken_numberSign = function() {
    ...
    c(t = this.fullCharCodeAtPos(), !0) || 92 === t))
...
Ut.getTokenFromCode = function(t) {
    ...
    case 40:
        return ++this.pos,
        this.finishToken(v.parenL);
    case 41:
        return ++this.pos,
        this.finishToken(v.parenR);
    case 59:
        return ++this.pos,
        this.finishToken(v.semi);
    case 44:
        return ++this.pos,
        this.finishToken(v.comma);
    case 91:
        return ++this.pos,
        this.finishToken(v.bracketL);
    case 93:
        return ++this.pos,
        this.finishToken(v.bracketR);
    case 123:
        return ++this.pos,
        this.finishToken(v.braceL);
    case 125:
        return ++this.pos,
        this.finishToken(v.braceR);
    case 58:
        return ++this.pos,
        this.finishToken(v.colon);
    case 96:
        if (this.options.ecmaVersion < 6)
            break;
        return ++this.pos,
        this.finishToken(v.backQuote);
    case 48:
        var e = this.input.charCodeAt(this.pos + 1);
        if (120 === e || 88 === e)
...
Ut.readRegexp = function() {
    for (var t, e, i = this.pos; ; ) {
        ...
        if (y.test(s) && this.raise(i, "Unterminated regular expression"),
        t)
            t = !1;
        else {
            if ("[" === s)
...
Ut.readInt = function(t, e, i) {
    for (var s = this.options.ecmaVersion >= 12 && void 0 === e, r = i && 48 === this.input.charCodeAt(this.pos), a = this.pos, n = 0, o = 0, h = 0, p = null == e ? 1 / 0 : e; h < p; ++h,
    ++this.pos) {
        ...
        if (s && 95 === c)
            ...
            if ((l = c >= 97 ? c - 97 + 10 : c >= 65 ? c - 65 + 10 : c >= 48 && c <= 57 ? c - 48 : 1 / 0) >= t)
                break;
            o = c,
            n = n * t + l
        }
    }
    return s && 95 === o && this.raiseRecoverable(this.pos - 1, "Numeric separator is not allowed at the last of digits"),
...
Ut.readRadixNumber = function(t) {
    ...
    this.options.ecmaVersion >= 11 && 110 === this.input.charCodeAt(this.pos) ? (i = qt(this.input.slice(e, this.pos)),
    ++this.pos) : c(this.fullCharCodeAtPos()) && this.raise(this.pos, "Identifier directly after number"),
...
Ut.readNumber = function(t) {
    ...
    c(this.fullCharCodeAtPos()) && this.raise(this.pos, "Identifier directly after number");
    var a, n = (a = this.input.slice(e, this.pos),
...
Ut.readString = function(t) {
    for (var e = "", i = ++this.pos; ; ) {
        ...
        i = this.pos) : 8232 === s || 8233 === s ? (this.options.ecmaVersion < 10 && this.raise(this.start, "Unterminated string constant"),
        ++this.pos,
        this.options.locations && (this.curLine++,
        this.lineStart = this.pos)) : (_(s) && this.raise(this.start, "Unterminated string constant"),
        ++this.pos)
    }
...
Ut.readTmplToken = function() {
    for (var t = "", e = this.pos; ; ) {
        ...
        if (96 === i || 36 === i && 123 === this.input.charCodeAt(this.pos + 1))
            return this.pos !== this.start || this.type !== v.template && this.type !== v.invalidTemplate ? (t += this.input.slice(e, this.pos),
            this.finishToken(v.template, t)) : 36 === i ? (this.pos += 2,
            this.finishToken(v.dollarBraceL)) : (++this.pos,
            this.finishToken(v.backQuote));
        if (92 === i)
    ...
    default:
        if (e >= 48 && e <= 55) {
            var s = this.input.substr(this.pos - 1, 3).match(/^[0-7]+/)[0]
              , r = parseInt(s, 8);
            return r > 255 && (s = s.slice(0, -1),
            r = parseInt(s, 8)),
            this.pos += s.length - 1,
            e = this.input.charCodeAt(this.pos),
            "0" === s && 56 !== e && 57 !== e || !this.strict && !t || this.invalidStringToken(this.pos - 1 - s.length, t ? "Octal literal in template string" : "Octal literal in strict mode"),
...
Ut.readWord1 = function() {
    ...
    for (var t = "", e = !0, i = this.pos, s = this.options.ecmaVersion >= 6; this.pos < this.input.length; ) {
        ...
        if (l(r, s))
            ...
            (e ? c : l)(n, s) || this.invalidStringToken(a, "Invalid Unicode escape"),
            t += V(n),
            i = this.pos
        }
        e = !1
...
Ut.readWord = function() {
    var t = this.readWord1()
      , e = v.name;
    return this.keywords.test(t) && (e = x[t]),
...
const Gt = q
  ...
  , Wt = q.parse.bind(q);
export {ct as Node, Gt as Parser, R as SourceLocation, Ft as Token, O as defaultOptions, D as getLineInfo, _ as isNewLine, y as lineBreak, b as lineBreakG, Wt as parse, it as tokContexts, v as tokTypes, Ht as tokenizer};
